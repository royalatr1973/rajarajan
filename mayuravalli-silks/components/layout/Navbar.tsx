...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-silk-dark/95 backdrop-blur-md shadow-lg' : 'bg-silk-dark'}`}>
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <Link href="/" className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              <span className="text-cream text-lg font-bold leading-tight">{settings.business_name}</span>
              <span className="text-gold/70 text-[10px] tracking-widest uppercase hidden sm:block">Pure Silk Sarees</span>
            </div>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            >
              <button className="text-cream hover:text-gold transition-colors duration-200 flex items-center gap-1">
                Collections <ChevronDown className={`h-4 w-4 transition-transform ${isProductsOpen ? 'rotate-180' : ''}`} />
              </button>
              {isProductsOpen && (
                <div className="absolute top-full left-0 mt-1 bg-white rounded-xl shadow-xl py-2 min-w-[220px] border border-cream-dark">
                  <Link href="/products" className="block px-4 py-2 text-silk-dark hover:bg-gold-light transition-colors">
                    All Collections
                  </Link>
                  <div className="border-t border-cream-dark my-1" />
                  {ALL_CATEGORIES.map(cat => (
                    <Link key={cat} href={`/products/${cat}`}
                      className="block px-4 py-2 text-silk-dark hover:bg-gold-light transition-colors">
            ...
            <Link href="/admin" className="bg-gold text-silk-dark px-4 py-2 rounded-lg font-semibold hover:bg-gold-light transition-colors duration-200">
              Admin
            </Link>
          </div>
      ...
      {/* Mobile Navigation */}
      {isOpen && (
        <div className="md:hidden bg-silk-dark border-t border-silk-medium">
          <div className="px-4 py-3 space-y-2">
            <Link href="/" className="block text-cream hover:text-gold py-2" onClick={() => setIsOpen(false)}>Home</Link>
            <Link href="/products" className="block text-cream hover:text-gold py-2" onClick={() => setIsOpen(false)}>All Collections</Link>
            {ALL_CATEGORIES.map(cat => (
              <Link key={cat} href={`/products/${cat}`}
            ...
            <Link href="/admin" className="block bg-gold text-silk-dark px-4 py-2 rounded-lg font-semibold text-center mt-2" onClick={() => setIsOpen(false)}>Admin</Link>
          </div>
        </div>
      )}
    </nav>
