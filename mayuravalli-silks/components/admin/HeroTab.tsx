...
export default function HeroTab() {
  ...
  async function handleSave() {
    ...
    } catch { setMessage('Error saving hero section') }
    setSaving(false)
  }
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setTimeout(() => setMessage(''), 3000)
    } catch { setMessage('Error uploading image. Make sure the "images" bucket exists.') }
    setUploading(false)
  }
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-silk-dark">Hero Section</h2>
      ...
      {/* Preview */}
      <div className="bg-gradient-to-br from-silk-dark via-silk-medium to-silk-dark rounded-2xl p-8 text-cream">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-cream-dark">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-silk-dark mb-1">Title</label>
            <input type="text" value={hero.title} onChange={e => update('title', e.target.value)}
          ...
          <div>
            <label className="block text-sm font-medium text-silk-dark mb-1">Subtitle</label>
            <textarea rows={3} value={hero.subtitle} onChange={e => update('subtitle', e.target.value)}
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-cream-dark">
        ...
        <div className="flex items-center gap-4">
          ...
          <input type="text" value={hero.image_url} placeholder="Or enter image URL" onChange={e => update('image_url', e.target.value)}
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-cream-dark">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-silk-dark mb-1">Primary Button Text</label>
            <input type="text" value={hero.cta_primary_text} onChange={e => update('cta_primary_text', e.target.value)}
              className="w-full px-4 py-2 rounded-lg border border-cream-dark focus:border-gold focus:ring-1 focus:ring-gold outline-none" />
          </div>
          <div>
            <label className="block text-sm font-medium text-silk-dark mb-1">Primary Button Link</label>
            <input type="text" value={hero.cta_primary_link} onChange={e => update('cta_primary_link', e.target.value)}
              className="w-full px-4 py-2 rounded-lg border border-cream-dark focus:border-gold focus:ring-1 focus:ring-gold outline-none" />
          </div>
          <div>
            <label className="block text-sm font-medium text-silk-dark mb-1">Secondary Button Text</label>
            <input type="text" value={hero.cta_secondary_text} onChange={e => update('cta_secondary_text', e.target.value)}
              className="w-full px-4 py-2 rounded-lg border border-cream-dark focus:border-gold focus:ring-1 focus:ring-gold outline-none" />
          </div>
          <div>
            <label className="block text-sm font-medium text-silk-dark mb-1">Secondary Button Link</label>
            <input type="text" value={hero.cta_secondary_link} onChange={e => update('cta_secondary_link', e.target.value)}
              className="w-full px-4 py-2 rounded-lg border border-cream-dark focus:border-gold focus:ring-1 focus:ring-gold outline-none" />
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={stat.vKey} className="space-y-2">
              <p className="text-xs font-medium text-silk-light uppercase">{stat.label}</p>
              <input type="text" value={hero[stat.vKey]} placeholder="Value (e.g., 100%)" onChange={e => update(stat.vKey, e.target.value)}
              ...
              <input type="text" value={hero[stat.lKey]} placeholder="Label (e.g., Pure Silk)" onChange={e => update(stat.lKey, e.target.value)}
                className="w-full px-3 py-2 rounded-lg border border-cream-dark focus:border-gold focus:ring-1 focus:ring-gold outline-none text-sm" />
            </div>
          ))}
        </div>
