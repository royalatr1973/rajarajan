...
const emptyProduct: Omit<Product, 'id'> = {
  name: '', description: '', category: 'kanchipuram-silk', type: '', image_url: '', price: '', original_price: '', is_featured: false, display_order: 0,
}

export default function ProductsTab() {
  const [products, setProducts] = useState<Product[]>([])
  const [activeCategory, setActiveCategory] = useState<Category>('kanchipuram-silk')
  ...
  async function fetchProducts() {
    if (!isSupabaseConfigured()) { setProducts(defaultProducts); return }
    try {
  ...
  function startEdit(product: Product) {
    ...
    setForm({
      ...
      original_price: product.original_price, is_featured: product.is_featured, display_order: product.display_order,
    })
    setShowForm(true)
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-silk-dark">Products</h2>
        <span className="text-sm text-silk-light">{products.length} total products</span>
      </div>
            ...
            <button key={cat}
              ...
              className={`px-4 py-2 rounded-xl font-medium text-sm transition-all ${
                activeCategory === cat
                  ...
                  : 'bg-white text-silk-light hover:bg-cream border border-cream-dark'
              }`}
            >
      ...
      {showForm && (
        <div className="bg-white rounded-2xl p-6 shadow-md border-2 border-gold/30">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-silk-dark">{editingId ? 'Edit' : 'Add'} Product</h3>
            <button onClick={resetForm} className="text-silk-light hover:text-silk-dark"><X className="h-5 w-5" /></button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-silk-dark mb-1">Name *</label>
            ...
            <div>
              <label className="block text-sm font-medium text-silk-dark mb-1">Type / Occasion</label>
              <input type="text" value={form.type} placeholder="e.g. Bridal, Traditional, Festival"
                onChange={e => setForm({ ...form, type: e.target.value })}
                className="w-full px-4 py-2 rounded-lg border border-cream-dark focus:border-gold focus:ring-1 focus:ring-gold outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-silk-dark mb-1">Price</label>
              <input type="text" value={form.price} placeholder="e.g. ₹18,500"
                onChange={e => setForm({ ...form, price: e.target.value })}
                className="w-full px-4 py-2 rounded-lg border border-cream-dark focus:border-gold focus:ring-1 focus:ring-gold outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-silk-dark mb-1">Original Price (for sale items)</label>
              <input type="text" value={form.original_price} placeholder="e.g. ₹22,000"
                onChange={e => setForm({ ...form, original_price: e.target.value })}
                className="w-full px-4 py-2 rounded-lg border border-cream-dark focus:border-gold focus:ring-1 focus:ring-gold outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-silk-dark mb-1">Display Order</label>
              <input type="number" value={form.display_order}
                onChange={e => setForm({ ...form, display_order: parseInt(e.target.value) || 0 })}
                className="w-full px-4 py-2 rounded-lg border border-cream-dark focus:border-gold focus:ring-1 focus:ring-gold outline-none" />
            </div>
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-silk-dark mb-1">Description</label>
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-silk-dark mb-1">Image</label>
            ...
            <div className="md:col-span-2">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm font-medium text-silk-dark">Featured Product (shows on homepage)</span>
              </label>
            </div>
          </div>
        ...
        {categoryProducts.map(product => (
          <div key={product.id} className="bg-white rounded-xl overflow-hidden shadow-sm border border-cream-dark hover:shadow-md transition-shadow">
            <div className="relative h-40 bg-cream">
              ...
              {product.is_featured && (
                <div className="absolute top-2 left-2 bg-gold text-silk-dark px-2 py-0.5 rounded-full text-xs font-semibold flex items-center gap-1">
                  <Star className="h-3 w-3 fill-current" /> Featured
                </div>
              )}
              <div className="absolute top-2 right-2 bg-silk-dark/70 text-cream px-2 py-0.5 rounded-full text-xs">
                {product.type}
              </div>
            </div>
            <div className="p-4">
              <h4 className="font-semibold text-silk-dark">{product.name}</h4>
              <p className="text-silk-light text-xs mt-1 line-clamp-2">{product.description}</p>
              <div className="flex items-center justify-between mt-3">
                <div className="flex items-center gap-2">
                  <span className="text-gold font-bold">{product.price}</span>
                  {product.original_price && product.original_price !== product.price && (
                    <span className="text-silk-light/50 text-xs line-through">{product.original_price}</span>
                  )}
                </div>
      ...
      {categoryProducts.length === 0 && (
        <div className="text-center py-12 bg-white rounded-2xl border border-cream-dark">
          ...
          <p className="text-silk-light">No {CATEGORY_INFO[activeCategory].label.toLowerCase()} sarees yet. Add your first one!</p>
        </div>
      )}
    </div>
  )
