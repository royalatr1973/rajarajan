...
import {
  ...
  Lock, LogOut, Gem, ChevronRight, Package, Users, Mail
} from 'lucide-react'
...
const ADMIN_PASSWORD = 'silkadmin2024'
...
function Dashboard() {
  ...
  return (
    <div className="space-y-8">
      <h2 className="text-2xl font-bold text-silk-dark">Dashboard</h2>
        ...
        {cards.map(card => {
          ...
          return (
            <div key={card.label} className="bg-white rounded-2xl p-6 shadow-sm border border-cream-dark">
              ...
              <p className="text-3xl font-bold text-silk-dark">{card.value}</p>
              <p className="text-sm text-silk-light mt-1">{card.label}</p>
            </div>
          )
        })}
      </div>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-cream-dark">
        <h3 className="text-lg font-semibold text-silk-dark mb-4">Quick Guide</h3>
        <div className="space-y-3 text-sm text-silk-light">
          ...
          <div className="flex items-start gap-3">
            <ChevronRight className="h-4 w-4 text-gold mt-0.5 shrink-0" />
            <p><strong>Products:</strong> Manage all sarees across 5 collections - Kanchipuram Silk, Soft Silk, Silk Cotton, Wedding, and Korvai.</p>
          </div>
          <div className="flex items-start gap-3">
            <ChevronRight className="h-4 w-4 text-gold mt-0.5 shrink-0" />
...
export default function AdminPage() {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-silk-dark via-silk-medium to-silk-dark flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md">
          <div className="text-center mb-8">
            <div className="bg-gold rounded-full p-3 w-16 h-16 flex items-center justify-center mx-auto mb-4">
              <Gem className="h-8 w-8 text-silk-dark" />
            </div>
            <h1 className="text-2xl font-bold text-silk-dark">Admin Panel</h1>
            <p className="text-silk-light mt-1">Mayuravalli Silks Management</p>
          </div>

          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-silk-dark mb-1">Password</label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-silk-light" />
                <input
                  type="password"
                  value={password}
            ...
            <button type="submit"
              className="w-full bg-gold text-silk-dark py-3 rounded-xl font-semibold hover:bg-gold-light transition-colors">
              Sign In
            </button>
          </form>

          <p className="text-center text-silk-light text-xs mt-6">
  ...
  return (
    <div className="min-h-screen bg-cream-light flex">
      {/* Sidebar */}
      <aside className={`${sidebarOpen ? 'w-64' : 'w-16'} bg-silk-dark text-cream transition-all duration-300 flex flex-col shrink-0`}>
        <div className="p-4 border-b border-silk-medium">
          <div className="flex items-center gap-2">
            <div className="bg-gold rounded-full p-1.5 shrink-0">
              <Gem className="h-5 w-5 text-silk-dark" />
            </div>
              ...
              <button key={tab.id}
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 transition-colors text-left ${
                  activeTab === tab.id
                    ...
                    : 'text-cream/70 hover:text-cream hover:bg-silk-medium/50'
                }`}
              >
        ...
        <div className="p-4 border-t border-silk-medium space-y-2">
