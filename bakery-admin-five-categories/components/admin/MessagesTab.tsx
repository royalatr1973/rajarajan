  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-cocoa-dark">Messages</h2>
          <p className="text-sm text-cocoa-light mt-1">
            {messages.length} total, {unreadCount} unread
          </p>
        </div>
        <button onClick={fetchMessages}
          className="bg-cream text-cocoa-dark px-4 py-2 rounded-xl font-medium hover:bg-cream-dark transition-colors flex items-center gap-2 text-sm border border-cream-dark">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-2xl border border-cream-dark">
          <Mail className="h-12 w-12 text-cocoa-light mx-auto mb-3" />
          <p className="text-cocoa-light">No messages yet. Messages from the contact form will appear here.</p>
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="font-semibold text-cocoa-dark">{msg.name}</span>
                    {!msg.is_read && (
                      <span className="bg-gold text-cocoa-dark px-2 py-0.5 rounded-full text-xs font-semibold">New</span>
                    )}
                  </div>
                  <div className="flex flex-wrap gap-4 text-xs text-cocoa-light mb-3">
                  ...
                  <p className="text-cocoa-medium text-sm leading-relaxed bg-cream-light p-3 rounded-lg">{msg.message}</p>
                </div>
