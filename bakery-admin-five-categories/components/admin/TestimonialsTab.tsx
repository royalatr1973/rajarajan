  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-cocoa-dark">Testimonials</h2>
        <span className="text-sm text-cocoa-light">{testimonials.length} testimonials</span>
      </div>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl p-6 shadow-md border-2 border-gold/30">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-cocoa-dark">{editingId ? 'Edit' : 'Add'} Testimonial</h3>
            <button onClick={resetForm} className="text-cocoa-light hover:text-cocoa-dark"><X className="h-5 w-5" /></button>
          </div>
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-cocoa-dark mb-1">Customer Name *</label>
            ...
            <div>
              <label className="block text-sm font-medium text-cocoa-dark mb-1">Testimonial Text *</label>
            ...
            <div>
              <label className="block text-sm font-medium text-cocoa-dark mb-1">Rating</label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {testimonials.map(t => (
          <div key={t.id} className="bg-white rounded-xl p-5 shadow-sm border border-cream-dark">
            ...
            <p className="text-cocoa-medium text-sm italic mb-3">&ldquo;{t.text}&rdquo;</p>
            <p className="text-cocoa-dark font-semibold text-sm">- {t.name}</p>
          </div>
        ))}
      </div>
    </div>
