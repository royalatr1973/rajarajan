  ...
  useEffect(() => {
    async function fetch() {
      if (!isSupabaseConfigured()) {
        setProducts(defaultProducts.filter(p => p.category === 'brownies'))
        setLoading(false)
        return
      }
      try {
        const { data } = await supabase.from('products').select('*').eq('category', 'brownies').order('display_order')
        if (data && data.length > 0) setProducts(data)
        else setProducts(defaultProducts.filter(p => p.category === 'brownies'))
      } catch {
        setProducts(defaultProducts.filter(p => p.category === 'brownies'))
      }
      setLoading(false)
      ...
      <main className="pt-16">
        <section className="bg-gradient-to-br from-cocoa-dark via-cocoa-medium to-cocoa-dark py-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <h1 className="text-4xl sm:text-5xl font-bold text-cream mb-4">Brownies</h1>
            <p className="text-cream/70 text-lg max-w-2xl mx-auto">
              Rich and fudgy brownies baked to perfection - the ultimate chocolate treat
            </p>
          </div>
        </section>
